-- CoolTrack Pro — Fix #2: get_orcamento_by_token usa coluna errada do profiles
-- ============================================================
-- A tabela `profiles` usa `id` como FK pra auth.users (não `user_id`).
-- O resto do app usa `.eq('id', user.id)` consistentemente — só esta função
-- estava com o nome errado. Erro: 42703 column p.user_id does not exist.
-- ============================================================
...
BEGIN
  ...
  -- Le profile como jsonb. FK pra auth.users e pela coluna `id` (nao user_id).
