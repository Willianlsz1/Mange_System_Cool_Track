...
export const Modal = {
  ...
  init() { document.querySelectorAll(".modal-overlay").forEach((el) => { el.addEventListener("click", (e) => { if (e.target === el) el.classList.remove("is-open"); }); }); },
};
...
export const Photos = {
  ...
  async add(input) {
    ...
    for (const file of files) { try { this.pending.push(await compressImage(file)); this.render(); } catch (e) { alert("Erro ao processar foto."); } }
...
export function renderHist() {
  ...
  el.innerHTML = `<div class="timeline">${list.map((r) => { const eq = findEquip(r.equipId); const dotMod = r.status !== "ok" ? `timeline__dot--${r.status}` : ""; return `<div class="timeline__item"><div class="timeline__dot ${dotMod}"></div><div class="timeline__item-inner"><div class="timeline__item-body"><div class="timeline__date">${Utils.formatDatetime(r.data)}</div><div class="timeline__title">${Utils.escapeHtml(r.tipo)}</div><div class="timeline__equip">📍 ${Utils.escapeHtml(eq?.nome ?? "—")} · ${Utils.escapeHtml(eq?.local ?? "")}</div><div class="timeline__obs">${Utils.escapeHtml(r.obs)}</div>${r.pecas ? `<div class="timeline__parts">🔩 ${Utils.escapeHtml(r.pecas)}</div>` : ""}${r.tecnico ? `<div class="timeline__parts">👷 ${Utils.escapeHtml(r.tecnico)}</div>` : ""}${r.proxima ? `<div class="timeline__next">📅 Próxima: ${Utils.formatDate(r.proxima)}</div>` : ""}</div><button class="timeline__delete" data-action="delete-reg" data-id="${r.id}" aria-label="Excluir registro"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18"/><path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"/><path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"/><line x1="10" x2="10" y1="11" y2="17"/><line x1="14" x2="14" y1="11" y2="17"/></svg></button></div></div>`; }).join("")}</div>`;
}
...
export const Equipamentos = {
  save() {
    const nome = Utils.getVal("eq-nome").trim(); const local = Utils.getVal("eq-local").trim();
    if (!nome || !local) return alert("Preencha nome e localização.");
    const rawTag = Utils.getVal("eq-tag").trim(); const normalizedTag = rawTag.toUpperCase();
    const { equipamentos } = getState();
    if (normalizedTag && equipamentos.some((e) => (e.tag || "").toUpperCase() === normalizedTag)) return alert("Já existe equipamento com esta TAG.");
    ...
    Modal.close("modal-add-eq"); Utils.clearVals("eq-nome", "eq-tag", "eq-local", "eq-modelo"); renderEquip(); updateHeader();
  },
  view(id) {
  ...
  delete(id) { setState((prev) => ({ ...prev, equipamentos: prev.equipamentos.filter((e) => e.id !== id), registros: prev.registros.filter((r) => r.equipId !== id) })); Modal.close("modal-eq-det"); renderEquip(); updateHeader(); },
};

export const Registro = {
  save() {
    const equipId = Utils.getVal("r-equip"); const data = Utils.getVal("r-data"); const tipo = Utils.getVal("r-tipo"); const obs = Utils.getVal("r-obs").trim(); const tecnico = Utils.getVal("r-tecnico").trim();
    const missing = [];
    if (!equipId) missing.push("Equipamento"); if (!data) missing.push("Data"); if (!tipo) missing.push("Tipo de Serviço"); if (!tecnico) missing.push("Técnico Responsável"); if (!obs || obs.length < 10) missing.push("Observações (mín. 10 caracteres)");
    if (missing.length > 0) return alert(`⚠️ Preencha os campos obrigatórios:\n\n• ${missing.join("\n• ")}`);
    const proxima = Utils.getVal("r-proxima");
    if (proxima && proxima < data.slice(0, 10)) return alert("A próxima manutenção não pode ser anterior à data do serviço.");
    ...
    this.clear(); alert("✅ Registro salvo!"); goView("historico");
  },
  clear(preserveEquip = false) {
    const toClear = ["r-tipo", "r-pecas", "r-obs", "r-proxima", "r-tecnico"];
    if (!preserveEquip) toClear.push("r-equip");
    Utils.clearVals(...toClear); Utils.setVal("r-status", "ok"); Utils.setVal("r-data", Utils.nowDatetime()); Photos.clear();
  },
};
...
export const Historico = {
  delete(id) {
    setState((prev) => {
      const regToDelete = prev.registros.find((r) => r.id === id); const registros = prev.registros.filter((r) => r.id !== id);
...
export function goView(name) {
  ...
  Utils.getEl(`view-${name}`)?.classList.add("active"); Utils.getEl(`nav-${name}`)?.classList.add("is-active");
  if (name === "inicio") renderInicio(); if (name === "equipamentos") renderEquip();
  if (name === "registro") { populateSelects(); Utils.setVal("r-data", Utils.nowDatetime()); }
  if (name === "historico") { populateSelects(); renderHist(); }
  if (name === "alertas") renderAlertas(); if (name === "relatorio") { populateSelects(); renderRelatorio(); }
  updateHeader(); window.scrollTo(0, 0);
}

export const Actions = {
  init() {
    // PARTE 3.1: Listener de aviso no formulário de registro
    ...
    // Redirecionamento vindo do modal de detalhes do equipamento
    document.addEventListener("click", (e) => {
      ...
      setTimeout(() => {
        ...
        // Dispara o aviso acima ao preencher automaticamente
